...
open class DatabaseService(private val services: ServiceHub) : SingletonSerializeAsToken() {
    ...
    private fun prepareStatement2(query: String): PreparedStatement {
        val session = services.jdbcSession()
        val preparedStatement = session.prepareStatement(query)
    ...
    protected fun executeQuery3(
            query: String): ArrayList<Any?> {
        val preparedStatement = prepareStatement2(query)

        return try {
            val resultSet = preparedStatement.executeQuery()


            val rw = resultSet.row
